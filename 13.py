...
class PolicyNet(torch.nn.Module):
    def __init__(self, state_dim, hidden_dim, action_dim, action_bound):
        ...
        self.action_bound = action_bound# 动作的界限
    def forward(self, x):
        x = F.relu(self.fc1(x)) 
        x = torch.tanh(self.fc2(x)) # 倍率
...
class QValueNet(torch.nn.Module):
    def __init__(self, state_dim, hidden_dim, action_dim):
        ...
        self.fc_out = torch.nn.Linear(hidden_dim, 1)# 评价Q值
...
class DDPG:
    ...
    def update(self, transition_dict):
        ...
        next_q_values = self.target_critic(next_states, self.target_actor(states))
