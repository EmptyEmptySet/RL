...
class GAIL:
    ...
    def learn(self, expert_s, expert_a, agent_s, agent_a, next_s, dones):
        expert_states = torch.tensor(expert_s, dtype=torch.float32).to(device)
        expert_actions = torch.tensor(expert_a).to(device)
        agent_states = torch.tensor(agent_s, dtype=torch.float32).to(device)
        ...
        agent_probs = self.discriminator(agent_states, agent_actions)
        discriminator_loss = nn.BCELoss()(agent_probs, torch.ones_like(agent_probs)) + nn.BCELoss()(expert_probs, torch.zeros_like(expert_probs))
        self.discriminator_optimizer.zero_grad()
        discriminator_loss.backward()
        self.discriminator_optimizer.step()
        rewards = -torch.log(agent_probs).detach().cpu().numpy()
        transition_dict = {
            'states': agent_s,
            'actions': agent_a,
            'rewards': rewards,
...
with tqdm(total=n_episode, desc="进度条") as pbar:
    for i in range(n_episode):
        ...
        while not done:
            action = agent.take_action(state)
            next_state, reward, xx, yy, _ = env.step(action)
            done = xx or yy
