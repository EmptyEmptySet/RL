...
class CQL:
    ...
    def update(self, transition_dict):
        ...
        temp_states = states.unsqueeze(1).repeat(1, self.num_random, 1).view(-1, states.shape[-1])
        temp_next_states = states.unsqueeze(1).repeat(1, self.num_random, 1).view(-1, next_states.shape[-1])
        random_curr_actions, random_curr_log_pi = self.actor(temp_states)
        random_next_actions, random_next_log_pi = self.actor(temp_next_states)
        q1_unif = self.critic_1(temp_states, random_unif_actions).view(-1, self.num_random, 1)
        q2_unif = self.critic_2(temp_states, random_unif_actions).view(-1, self.num_random, 1)
        q1_curr = self.critic_1(temp_states, random_curr_actions).view(-1, self.num_random, 1)
        q2_curr = self.critic_2(temp_states, random_curr_actions).view(-1, self.num_random, 1)
        q1_next = self.critic_1(temp_states, random_next_actions).view(-1, self.num_random, 1)
        q2_next = self.critic_2(temp_states, random_next_actions).view(-1, self.num_random, 1)
    ...
    with tqdm(total=int(num_epochs / 10), desc='Iteration %d' % i) as pbar:
        for i_epoch in range(int(num_epochs / 10)):
            ...
            while not done:
                ...
                next_state, reward, tempx, tempy, _ = env.step(action)
                done = tempx or tempy
                state = next_state
                epoch_return += reward
