...
class CEM:
    ...
    def optimize(self, state, init_mean, init_var):
        ...
        X = truncnorm(-2, 2, loc = np.zeros_like(mean), scale = np.ones_like(var)) #构建有截断的正态分布随机变量
        state = np.tile(state, (self.n_sequence, 1))
        for _ in range(5):
            lb_dist, ub_dist = mean - self.lower_bound, self.upper_bound - mean
            costrained_var = np.minimum(np.minimum(np.square(lb_dist / 2), np.square(ub_dist / 2)), var)
            action_sequences = [X.rvs() for _ in range(self.n_sequence)] * np.sqrt(costrained_var) + mean
    ...
    def predict(self, inputs, batch_size=64):
        ...
        for i in range(0, inputs.shape[0], batch_size):
            input = torch.from_numpy(
                inputs[i:min(i + batch_size, inputs.shape[0])]).float().to(device)
...
class PETS:
    ...
    def mpc(self):
        ...
        while not done:
            ...
            next_obs, reward, x, y, _ = self._env.step(action)
            done = x or y
